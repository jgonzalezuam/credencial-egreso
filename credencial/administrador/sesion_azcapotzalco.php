...
!empty ($_POST['unidad'])   ){
  ...
  $emailAzc             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['email'],'UTF-8')));
  $unidadAzc             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['unidad'],'UTF-8')));
...
from
admi_unidades
where admi_unidades.email    = '$emailAzc'
and   admi_unidades.unidad = '$unidadAzc'";
...
if($contarFilas <1 || $contarFilas > 1 ){  
  header("location: usuario_no_existe_azcapotzalco.php");
  exit();
}
elseif($contarFilas == 1){	
  ...
  while ($datos       = $resultado->fetch_array()){
    ...
    if ($sesion_activa === '0') {
      ...
      session_start(); 
      $_SESSION['nombreAzc']   =  $nombreCompleto;
      $_SESSION['emailAzc']           =  $email;
      $_SESSION['unidadAZc']           =  $unidad;
      $_SESSION['idAzc']              =  $id;
    ...
    if ($sesion_activa === '1') {
      ...
      header("location: sesion_activa_azc.php");
    }
