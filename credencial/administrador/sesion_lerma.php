...
!empty ($_POST['unidad'])   ){
  ...
  $emailLer             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['email'],'UTF-8')));
  $unidadLer             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['unidad'],'UTF-8')));
...
from
admi_unidades
where admi_unidades.email    = '$emailLer'
and   admi_unidades.unidad = '$unidadLer'";
...
if($contarFilas <1 || $contarFilas > 1 ){  
  header("location: usuario_no_existe_lerma.php");
  exit();
}
elseif($contarFilas == 1){	
  ...
  while ($datos       = $resultado->fetch_array()){
    ...
    if ($sesion_activa === '0') {
      ...
      session_start(); 
      $_SESSION['nombreLer']   =  $nombreCompleto;
      $_SESSION['emailLer']           =  $email;
      $_SESSION['unidadLer']           =  $unidad;
      $_SESSION['idLer']              =  $id;
    ...
    if ($sesion_activa === '1') {
      ...
      header("location: sesion_activa_ler.php");
    }
