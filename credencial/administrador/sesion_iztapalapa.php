...
!empty ($_POST['unidad'])   ){
  ...
  $emailIzt             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['email'],'UTF-8')));
  $unidadIzt             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['unidad'],'UTF-8')));
...
from
admi_unidades
where admi_unidades.email    = '$emailIzt'
and   admi_unidades.unidad = '$unidadIzt'";
...
if($contarFilas <1 || $contarFilas > 1 ){  
  header("location: usuario_no_existe_iztapalapa.php");
  exit();
}
elseif($contarFilas == 1){	
  ...
  while ($datos       = $resultado->fetch_array()){
    ...
    if ($sesion_activa === '0') {
      ...
      session_start(); 
      $_SESSION['nombreIzt']   =  $nombreCompleto;
      $_SESSION['emailIzt']           =  $email;
      $_SESSION['unidadIzt']           =  $unidad;
      $_SESSION['idIzt']              =  $id;
    ...
    if ($sesion_activa === '1') {
      ...
      header("location: sesion_activa_izt.php");
    }
