...
!empty ($_POST['unidad'])   ){
  ...
  $emailRec             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['email'],'UTF-8')));
  $unidadRec             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['unidad'],'UTF-8')));
...
from
admi_unidades
where admi_unidades.email    = '$emailRec'
and   admi_unidades.unidad = '$unidadRec'";
...
if($contarFilas <1 || $contarFilas > 1 ){  
  header("location: usuario_no_existe_rectoria.php");
  exit();
}
elseif($contarFilas == 1){	
  ...
  while ($datos       = $resultado->fetch_array()){
    ...
    if ($sesion_activa === '0') {
      ...
      session_start(); 
      $_SESSION['nombreRec']   =  $nombreCompleto;
      $_SESSION['emailRec']           =  $email;
      $_SESSION['unidadRec']           =  $unidad;
      $_SESSION['idRec']              =  $id;
    ...
    if ($sesion_activa === '1') {
      ...
      header("location: sesion_activa_rec.php");
    }
