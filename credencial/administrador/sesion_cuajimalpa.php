...
!empty ($_POST['unidad'])   ){
  ...
  $emailCua             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['email'],'UTF-8')));
  $unidadCua             = mysqli_real_escape_string($mysqli, mb_strtolower(trim($_POST['unidad'],'UTF-8')));
...
from
admi_unidades
where admi_unidades.email    = '$emailCua'
and   admi_unidades.unidad = '$unidadCua'";
...
if($contarFilas <1 || $contarFilas > 1 ){  
  header("location: usuario_no_existe_cuajimalpa.php");
  exit();
}
elseif($contarFilas == 1){	
  ...
  while ($datos       = $resultado->fetch_array()){
    ...
    if ($sesion_activa === '0') {
      ...
      session_start(); 
      $_SESSION['nombreCua']   =  $nombreCompleto;
      $_SESSION['emailCua']           =  $email;
      $_SESSION['unidadCua']           =  $unidad;
      $_SESSION['idCua']              =  $id;
    ...
    if ($sesion_activa === '1') {
      ...
      header("location: sesion_activa_cua.php");
    }
