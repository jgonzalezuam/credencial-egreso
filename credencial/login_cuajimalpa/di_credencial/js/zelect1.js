/*
  ...
  opts:
    ...
    placeholder:    String/DOM/jQuery: placeholder text/html before anything is selected
                      zelect1 automatically selects first item if not provided
...
(function($) {
  ...
  $.fn.zelect1 = function(opts) {
    opts = $.extend({}, defaults, opts)

    return this.each(function() {
      if ($(this).parent().length === 0) throw new Error('<select> element must have a parent')
      var $select = $(this).hide().data('zelect1Item', selectItem).data('refreshItem', refreshItem).data('reset', reset)

      var $zelect1 = $('<div>').addClass('zelect1')
      var $selected = $('<div>').addClass('zelect1ed')
      ...
      $search.keyup(function(e) {
        switch (e.which) {
          ...
          case keys.enter:
            var curr = listNavigator.current().data('zelect1-item')
      ...
      $list.on('click', 'li', function() { selectItem($(this).data('zelect1-item')) })
      $zelect1.mouseenter(function() { $zelect1.addClass('hover') })
      $zelect1.mouseleave(function() { $zelect1.removeClass('hover') })
      $zelect1.attr("tabindex", $select.attr("tabindex"))
      $zelect1.blur(function() { if (!$zelect1.hasClass('hover')) hide() })
      $search.blur(function() { if (!$zelect1.hasClass('hover')) hide() })

      $selected.click(toggle)

      $zelect1.insertAfter($select)
        .append($selected)
      ...
      function refreshItem(item, identityCheckFn) {
        ...
        if (eq($select.data('zelect1ed'), item)) {
          renderContent($selected, opts.renderItem(item))
          $select.data('zelect1ed', item)
        }
        var term = searchTerm()
        $list.find('li').each(function() {
          if (eq($(this).data('zelect1-item'), item)) {
            renderContent($(this), opts.renderItem(item, term)).data('zelect1-item', item)
          }
        })
      }
      ...
      function appendItem(item, term) {
        $list.append(renderContent($('<li>').data('zelect1-item', item), opts.renderItem(item, term)))
      }
      ...
      function initialSelection(useOptsInitial) {
        ...
        if (useOptsInitial && opts.initial) {
          ...
          selectItem($list.children().eq($s.index()).data('zelect1-item'))
        } else if (opts.placeholder) {
          $selected.html(opts.placeholder).addClass('placeholder')
        } else {
          var first = $list.find(':first').data('zelect1-item')
  ...
  $.fn.zelect1Item = callInstance('zelect1Item')
  $.fn.refreshzelect1Item = callInstance('refreshItem')
  $.fn.resetzelect1 = callInstance('reset')
